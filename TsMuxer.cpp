...
int  TsMuxer::Test() {
    ...
    in_filename  = "c:\\ad2.mp4";
    out_filename = "c:\\test.flv";
...
static int write_packet(void *opaque, uint8_t *buf, int buf_size)
{
    ...
    int c = (int)buf[2];
    int d = (int)buf[3];
    printf("write pkt to network,%X,%X,%X,%X,%d\n",(int)buf[0],(int)buf[1],(int)buf[2],(int)buf[3],buf_size);
    return 0;
}

int TsMuxer::TestWriteToMem(){
    AVOutputFormat *ofmt = NULL;
    AVFormatContext *ifmt_ctx = NULL, *ofmt_ctx = NULL;
    AVPacket pkt;
    const char *in_filename, *out_filename;
    ...
    if (!(ofmt->flags & AVFMT_NOFILE)) {
        ...
        size_t buffer_size, avio_ctx_buffer_size = 128*1024;
...
int TsMuxer::PushToNetwork(){
    ...
    //filename = "c:\\test.mp4";
    filename = "srt://172.21.22.12:8080?streamid=push.sls.com/live/zjb&latency=400000&transtype=live";
    //filename = "rtmp://172.21.22.12/live/zjb";
    ...
    while (encode_video || encode_audio) {
        ...
        if(encode_video){
            if(duration.count()>0&&duration.count()<40){
                printf("sleep for %d ms\n",(int)(40-duration.count()));
